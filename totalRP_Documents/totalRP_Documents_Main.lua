...
function ReceiveDocument(docu,sender)
	...
	if TRP_Module_Documents[ID] ~= nil then
		...
		elseif etape == 10 then -- Fin de transaction
			...
			TRPSecureSendAddonMessage("SDM","{g}"..Joueur.." has succefully receive your document \""..TRP_Module_Documents[ID]["VignetteTitre"].."\".",sender);
			if TRPWaitingForInfos then
				TRPWaitingForInfos = nil;
				sendMessage("{g}You succefully receive the document \""..TRP_Module_Documents[ID]["VignetteTitre"].."\" from "..sender..".");
			else -- Notify updates to others
				sendMessage("{g}You succefully got updated the document \""..TRP_Module_Documents[ID]["VignetteTitre"].."\" from "..sender..".");
